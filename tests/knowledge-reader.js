...
(async () => {
     ...
     * links stayed broken. */
    {
        ...
        // with no User-Agent is a downloader that fails on half the manifest.
        {
            ...
            // THE EPERM THAT KILLED EVERY FIRST DOWNLOAD: the mirror path was
            // computed with resolveInRoot, which realpaths the ROOT — and the
            // mirror does not exist until the first download creates it, so on
            // every fresh install the resolver threw and a silent catch fell
            // back to the READ-ONLY resources path (mkdir under Program Files,
            // EPERM). The write target is computed lexically now, and a
            // containment violation is an ERROR — never a fallback to a
            // location that cannot be written.
            check("the mirror write path never requires the mirror to exist, and " +
                  "NEVER falls back to resources/ — the silent fallback is the " +
                  "read-only-dir bug resurrected through a throw",
                /that path escapes the download mirror/.test(msrc)
                && !/catch { cacheFull = full; }/.test(msrc), null);
        }
    }
