    ...
    go.addEventListener("click", async () => {
        ...
        if (window.lcl.onPatchProgress && !showPatchBanner._progressWired) {
            ...
            window.lcl.onPatchProgress((pr) => {
                ...
                if (!l) return;
                if (pr && pr.pct >= 100) {
                    l.innerText = "verifying — then approve the Windows prompt";
...
function buildLibraryGroup(lib) {
    ...
    if (!lib.addedByUser && lib.sourcesMissing > 0) {
        ...
        // knowledge sources." Each fetch goes through the SAME operator-approved
        // handler as the single download, sequentially — one slow host must not
        // stampede sixty parallel sockets — into the same writable mirror. The
        // outcome is numbers, not adjectives: downloaded / failed, with the
        // failures named and the rest pointed at the error log.
        const all = document.createElement("button");
        all.className = "primary small kb-fetch-all";
        all.innerText = "Download all (" + lib.sourcesMissing + ")";
        ...
        all.addEventListener("click", async () => {
            ...
            if (typeof fn !== "function") { all.innerText = "not available in this build"; return; }
            all.disabled = true;
            const want = lib.docs.filter(d => !d.sourceOnDisk && !d.extracted && d.sourceUrl);
            let done = 0; const failed = []; let netOff = false;
            for (const d of want) {
                all.innerText = "Downloading " + (done + failed.length + 1) + "/" + want.length +
                    " — " + String(d.title || d.file).slice(0, 36) + "…";
                const r = await fn(d.id, { approved: true })
                    .catch(e => ({ ok: false, error: String((e && e.message) || e) }));
                if (r && r.ok) { done++; continue; }
                if (r && r.blocked === "network") { netOff = true; break; }
                failed.push({ title: String(d.title || d.file), error: (r && (r.error || r.reason)) || "failed" });
            }
            all.innerText = "Download all";
            const note = document.createElement("div");
            note.className = "kb-group-note" + ((failed.length || netOff) ? " stale" : "");
            note.innerText = netOff
            ...
            kbFetchAllNote = { libId: lib.id, text: note.innerText,
